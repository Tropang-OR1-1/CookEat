...
router.get('/feed/posts', justifyToken, upload.none(), async (req, res) => {
    ...
    const query = `
        ...
        GROUP BY p.id, r.public_id, u.public_id, u.username, um.fname
...
let query = `
    SELECT 
        ...
        u.public_id AS author_public_id,
        u.username AS author_username,
        -- Aggregate one profile picture filename or NULL
        MAX(m.fname) AS author_picture,  
    ...
    LEFT JOIN tags t ON t.id = pt.tags_id
    LEFT JOIN user_profile u ON p.user_id = u.id
    LEFT JOIN usermedia m ON m.user_id = u.id AND m.type = 'profile'  -- LEFT JOIN for profile picture
    ...
    GROUP BY p.id, r.public_id, u.public_id, u.username, u.biography
`;
...
router.get('/user/recipes/:public_id', justifyToken, upload.none(), async (req, res) => {
    ...
    if (!isValidUUID(public_id))
      ...
      const userResult = await db.query(
        `SELECT id, username FROM user_profile WHERE public_id = $1`,
        [public_id]
      );
